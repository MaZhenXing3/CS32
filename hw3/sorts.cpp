...
struct Sensor
{
    IdType id;
    double avg;
    double readings[NREADINGS]; // reading is array holding doubles;
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    // two for loops to do the insertion sort
    int it;
    for (int i = 1; i < s.size(); i++){
        it = i;
        for (int j = i-1 ; j >= 0; j--){
            // get the last one's value -- s[i] is the last;
            // compare it with the ones before it using s[j];
            // if comp(s[j], s[i]): swap(s[j], s[i]);
            if (comp(s[it],s[j])) // last one greater than one before it
            {
                swap(s[j], s[it]);
                it = j;
            }
...
int main()
{
    ...
    // this shuffles the ids' order eg. if total 5, then ids can be [0,3,4,2,1]
    random_shuffle(ids.begin(), ids.end());  // from <algorithm>
    
      // Create a bunch of Sensors and keep them in a vector
    vector<Sensor> unorderedSensors;
    for (size_t k = 0; k < ids.size(); k++)
    //when call Sensor(ids[k])(where ids[k] is not k because shuffled),
    // a Sensor constructor will be called and fill up the Sensor's avg and readings
    ...
    // ###########  1st test
    // using copy constructor in vector initialization.
